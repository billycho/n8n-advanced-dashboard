...
export default function AIAgentDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: workflow, isLoading: isWorkflowLoading } = useN8NWorkflow(agent?.workflow_id || "");
  const { mutate: updateWorkflow } = useUpdateN8NWorkflow();
  ...
  const handleUpdateSchedule = () => {
    if (!agent?.workflow_id || !workflow) return;
  ...
  const handleAddParam = () => {
    ...
    let newParams;
    if (editingParam) {
      newParams = parameters.map(p => p.id === editingParam.id ? { ...p, name: paramKey, value: paramValue } : p);
    } else {
    ...
    updateWorkflow({
      ...
      onSuccess: () => {
        ...
        setParamKey("");
        setParamValue("");
        toast.success(editingParam ? "Parameter updated" : "Parameter added");
      },
      onError: (error) => {
